		...
		<section class="md:col-span-7 lg:col-span-8 space-y-4 min-w-0">
			...
			{#if volumeStats}
				...
				<div class="card variant-glass-surface p-4">
					<div class="flex items-start justify-between gap-3">
						<div>
							<h3 class="text-base font-semibold">Recent set schemes</h3>
							<p class="text-sm opacity-70">What you last did (quick recall).</p>
						</div>
					</div>
					<div class="mt-3 flex flex-wrap gap-2">
						{#each volumeStats.weekly_volume.at(-1)?.set_schemes || [] as scheme}
							<span class="badge variant-soft text-xs">{scheme}</span>
						{/each}
					</div>
				</div>
			{/if}
